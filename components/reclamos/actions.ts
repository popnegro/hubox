...
export type Analysis = z.infer<typeof AnalysisSchema>;
 ...
 async function getAIAnalysis(claimText: string): Promise<Analysis> {
   console.log("Llamando a la API de IA para analizar:", claimText);
 
   const OLLAMA_URL = process.env.OLLAMA_URL || "http://localhost:11434/api/chat";
   const OLLAMA_MODEL = process.env.OLLAMA_MODEL || "llama3";
   const API_KEY = process.env.OLLAMA_API_KEY; // Clave para servicios hosteados
   ...
   try {
     const response = await fetch(OLLAMA_URL, {
       ...
       headers: { 
         ...
         // Añade el header de autorización si la API Key existe
       ...
       body: JSON.stringify({
         ...
         response_format: { "type": "json_object" }, // Para APIs compatibles con OpenAI como Groq
         stream: false,
       }),
     });
